.MODEL TINY ; для создания COM файла
.186        ; для эмулятора процессора

CodeSeg SEGMENT
	ASSUME CS:CodeSeg, DS:CodeSeg              ; CS и DS указывают на сегмент кода
    ORG 100h                                   ; размер PSP(Префикс программного сегмента) - структура данных, которая используется в 
                                               ; DOS для сохранения состояния программы.
...
MAIN:   
    ...
    OLD_BREAKING dd ? ; адресс старого прерывания  (адрес сегмента 2 байта адрес смещения 2 байта)
    FLAG db 228
    
    
MY_BREAKING:
    pusha             ; сохраняет в стеке содержимое 16-битных РОН (AX, CX, DX, BX, SP, BP, SI, DI.)
    ...
    dec SPEED         ; уменьшаем вывод на 1 символ (30 символов - самый медленный)
